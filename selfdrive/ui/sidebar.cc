...
static void ui_draw_sidebar_temp_metric(UIState *s) {
  ...
  ui_draw_sidebar_metric(s, _("TEMP"), temp_val.c_str(), temp_severity_map[s->scene.thermal.getThermalStatus()], 0, NULL);
}
...
static void ui_draw_sidebar_panda_metric(UIState *s) {
  ...
  std::string panda_message = _("VEHICLE\nONLINE");
...
static void ui_draw_sidebar_connectivity(UIState *s) {
  static std::map<NetStatus, std::pair<const char *, int>> connectivity_map = {
    {NET_ERROR, {_("CONNECT\nERROR"), 2}},
    {NET_CONNECTED, {_("CONNECT\nONLINE"), 0}},
    {NET_DISCONNECTED, {_("CONNECT\nOFFLINE"), 1}},
  };
